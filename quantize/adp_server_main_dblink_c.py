...
def model_compose(params_dict, ratio=0.5):
    global_params_dict = copy.deepcopy(params_dict)
    for k, v in global_params_dict.items():
        if v.dim() == 2 and v.shape[0] > 1 and v.shape[1] > 1:
            new_weight = decompose_tensor(v, ratio)
            global_params_dict[k], model_size_MB = recompose_tensor(new_weight[0],new_weight[1],new_weight[2])
...
def main():
    ...
    for epoch_idx in range(1, 1 + cfg['epoch_num']):
        ...
        global_level = 32
        Global_quantizate = False
        Global_compose = False
        if Global_quantizate:
            new_server_model    = server_quantization(global_model.state_dict(), level=global_level)
        elif Global_compose:
            new_server_model    = model_compose(global_model.state_dict(), ratio=(global_level/32))
        else:
            new_server_model    = global_model.state_dict()
...
def decompose_tensor(weight, ratio):
    # 奇异值分解
    U, S, V = torch.svd(weight)
    # 压缩
    #print(ratio)
    rank = math.ceil(ratio * min(U.shape[0], U.shape[1]))
    #print(rank)
    U1 = U[: , : rank]
    S1 = S[: rank]
    V1 = V[: , : rank]
...
def recompose_tensor(U1, S1, V1):
    # 创建对角矩阵
    S_diag = torch.diag(S1)
    # 重新组合
    approx_weight = U1 @ S_diag @ V1.t()

    bytes_per_element = U1.element_size()  # for torch.float32, it returns 4
    size_U1 = U1.numel() * bytes_per_element
    size_S1 = S1.numel() * bytes_per_element
    size_V1 = V1.numel() * bytes_per_element

    total_size = (size_U1 + size_S1 + size_V1) / (1024 ** 2)

    return approx_weight, total_size
